    ...
    steps:
      ...
      - name: Set up Python
        ...
        with:
          python-version: "3.12"
      - name: Run Bandit
        run: bash <(curl -s https://raw.githubusercontent.com/lupaxa-cicd-toolbox/bandit/master/src/pipeline.sh)
```
...
```bash
bash <(curl -s https://raw.githubusercontent.com/lupaxa-cicd-toolbox/bandit/master/src/pipeline.sh)
```
